...
tmap_save(map_StdHT, "Site_Index.png", height=7)

# save the interactive map in html file
tmap_save(map_StdHT, "Site_Index.html")
...
# Save the map  
tmap_save(Queen_Neighbour, "Site_Index Neighborhood Queen.png", height=7)
tmap_save(Queen_Neighbour, "siteindexneighborhoodqueen.html")

#Defining Neighborhood using Rook's case
...
# Save maps  
tmap_save(Neighbours, "Site_Index Neighbourhood Queen and Rook.png", height=7)
tmap_save(Neighbours, "siteindexqueenandrook.html")
...
mi <- moran.test(vriClean$Site_Index, vri.lw, zero.policy = TRUE)
mi 
...
mi2 <- moran.test(vriClean$Site_Index, vri.lw2, zero.policy = TRUE)
mi2 
...
lisa.test <- localmoran(vriClean$Site_Index, vri.lw, zero.policy = TRUE)
summary(lisa.test)
...
LISAzvalue = subset(vriClean, P < 0.005) #Create new object with only P < 0.005 
map_LISAzvalue <- tm_shape(vriClean) +   tm_polygons(col = "lightgrey") +
  tm_shape(LISAzvalue) + 
  tm_polygons(col = "Z.Ii", 
              title = "Site Index LISA Z Value (Queen's)", 
              style = "fisher", 
              palette = "RdBu", n = 9,) +
  tm_add_legend(type = "symbol", labels = c("Not significant"), col = c(adjustcolor("lightgrey", alpha.f = 1)), shape = c(22))
map_LISAzvalue
# Save the map  
tmap_save(map_LISAzvalue, "Site_Index LISA Z Value Queens.png", height=7)
tmap_save(map_LISAzvalue, "Site_Index LISA Z Value Queens.html")

#Plot the results of the LISA to a Scatter plot and save it as a png file 
png("LISA Moran Plot Queen's Case Site_Index.png")
mp <- moran.plot(vriClean$Site_Index, vri.lw, zero.policy=TRUE, spChk=NULL, labels=NULL, xlab="Site Index (location i) ", 
           ylab="Spatially Lagged Site Index (neighbourhood of location i)", quiet=NULL)
if (require(ggplot2, quietly=TRUE)) {
  ...
  ggplot(mp, aes(x=x, y=wx)) + geom_point(shape=1) + 
    ...
    xlab("Site Index (location i)") + ylab("Spatially Lagged Site Index (neighbourhood of location i)")
}
dev.off()

# Local Moran's I Analysis (Rooks's Neighborhood)
lisa.test2 <- localmoran(vriClean$Site_Index, vri.lw2, zero.policy = TRUE)
summary(lisa.test2)
...
map_LISAzvalue2 <- tm_shape(vriClean) +   tm_polygons(col = "lightgrey") +
  ...
  tm_polygons(col = "Z.Ii2", 
              title = "Site Index LISA Z Value (Rook's)", 
              style = "fisher", 
              palette = "RdBu", n = 9,) +
  tm_add_legend(type = "symbol", labels = c("Not significant"), col = c(adjustcolor("lightgrey", alpha.f = 1)), shape = c(22))
...
map_LISAzvalue2
# Save the map  
tmap_save(map_LISAzvalue2, "Site Index LISA Z Value Rooks.png", height=7)
tmap_save(map_LISAzvalue2, "Site Index LISA Z Value Rooks.html")

#Plot the results of the LISA to a Scatter plot and save it as a png file 
png("LISA Moran Plot Rook's Case Site Index.png")
mp2 <- moran.plot(vriClean$Site_Index, vri.lw2, zero.policy=TRUE, spChk=NULL, labels=NULL, xlab="Site_Index (location i) ", 
                 ylab="Spatially Lagged Site Index(neighbourhood of location i)", quiet=NULL)
if (require(ggplot2, quietly=TRUE)) {
